    ...
    for<'a> &'a GC2: GroupOpsBounds<'a, C2, GC2>,
{
    ...
    pub max_mu: usize, // max number of LCCCS instances to be folded // TODO maybe not 'max_{mu/nu}' and use directly '{mu/nu}'
    pub max_nu: usize, // max number of CCCS instances to be folded
    pub i: Option<CF1<C1>>,
    ...
    for<'a> &'a GC2: GroupOpsBounds<'a, C2, GC2>,
{
    ...
    pub fn empty(
        ...
        let mut augmented_f_circuit =
            Self::default(poseidon_config, F_circuit, initial_ccs, max_mu, max_nu);
        if ccs.is_some() {
    ...
    pub fn upper_bound_ccs(&self) -> Result<CCS<C1::ScalarField>, Error> {
        ...
        let n_iters = 2;
        for _ in 0..n_iters {
            let Us = vec![U_i.clone(); self.max_mu - 1];
            let Ws = vec![W_i.clone(); self.max_mu - 1];
            let us = vec![u_i.clone(); self.max_nu - 1];
            let ws = vec![w_i.clone(); self.max_nu - 1];

            let all_Us = vec![vec![U_i.clone()], Us.clone()].concat();
            let all_us = vec![vec![u_i.clone()], us.clone()].concat();
            let all_Ws = vec![vec![W_i.clone()], Ws].concat();
            let all_ws = vec![vec![w_i.clone()], ws].concat();
    ...
    for<'a> &'a GC2: GroupOpsBounds<'a, C2, GC2>,
{
    fn generate_constraints(self, cs: ConstraintSystemRef<CF1<C1>>) -> Result<(), SynthesisError> {
        ...
        let Us = Vec::<LCCCSVar<C1>>::new_witness(cs.clone(), || {
            Ok(self.Us.unwrap_or(vec![U_dummy.clone(); self.max_mu - 1]))
        })?;
        ...
        let nimfs_proof_dummy = NIMFSProof::<C1>::dummy(&self.ccs, self.max_mu, self.max_nu);
        let nimfs_proof = ProofVar::<C1>::new_witness(cs.clone(), || {
            Ok(self.nimfs_proof.unwrap_or(nimfs_proof_dummy))
        })?;

        let cf_u_dummy = CommittedInstance::dummy(cf_io_len(self.max_mu + self.max_nu));
        ...
        let all_Us = vec![vec![U_i.clone()], Us].concat();
        let all_us = vec![vec![u_i.clone()], us].concat();
        ...
        let cf_x: Vec<NonNativeUintVar<CF2<C2>>> = vec![
...
mod tests {
    ...
    pub fn test_augmented_f_circuit() {
        ...
        let mut augmented_f_circuit = AugmentedFCircuit::<
            ...
            CubicFCircuit<Fr>,
        >::empty(
            &poseidon_config, F_circuit, None, max_mu, max_nu
        )
        .unwrap();
        ...
        let cf_circuit = CycleFoldCircuit::<Projective, GVar>::empty(max_mu + max_nu);
        ...
        // TODO review the setup -cf_r1cs.l (same for main augmentedfcircuit)
        ...
        for i in 0..n_steps {
            ...
            let Us = vec![U_i.clone(); max_mu - 1];
            let Ws = vec![W_i.clone(); max_mu - 1];
            let us = vec![u_i.clone(); max_nu - 1];
            let ws = vec![w_i.clone(); max_nu - 1];
            let all_Us = vec![vec![U_i.clone()], Us.clone()].concat();
            let all_us = vec![vec![u_i.clone()], us.clone()].concat();
            let all_Ws = vec![vec![W_i.clone()], Ws].concat();
            let all_ws = vec![vec![w_i.clone()], ws].concat();
