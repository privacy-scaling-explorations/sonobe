    ...
    CS2: CommitmentScheme<C2>,
{
    ...
    pub rho_powers: Option<Vec<C1::ScalarField>>,
    /// CycleFold running instance
    pub cf_U_i: Option<NovaCommittedInstance<C2>>,
    pub cf_W_i: Option<NovaWitness<C2>>,
    ...
    <C1 as CurveGroup>::BaseField: PrimeField,
{
    pub fn from_hypernova<FC: FCircuit<C1::ScalarField>>(
        hn: HyperNova<C1, GC1, C2, GC2, FC, CS1, CS2>,
    ) -> Result<Self, Error> {
    ...
    fn generate_constraints(self, cs: ConstraintSystemRef<CF1<C1>>) -> Result<(), SynthesisError> {
        ...
        #[cfg(not(feature = "light-test"))]
        {
            ...
            use crate::folding::nova::{
                decider_eth_circuit::{R1CSVar, RelaxedR1CSGadget},
                NOVA_CF_N_POINTS,
            };
            use ark_r1cs_std::ToBitsGadget;

            let cf_u_dummy_native = NovaCommittedInstance::<C2>::dummy(cf_io_len(NOVA_CF_N_POINTS));
            let cf_w_dummy_native = NovaWitness::<C2>::new(
        ...
        // 8. verify the NIMFS.V of the final fold, and check that the obtained rho_powers from the
        ...
        // check that the obtained U_{i+1} from the NIMFS.V matches the U_{i+1} from the input,
        ...
        // Notice that rho_powers only contains one element, since at the final fold we are only
        // folding 2-to-1 instances.
        let external_rho_powers = Vec::<FpVar<CF1<C1>>>::new_input(cs.clone(), || {
            Ok(self.rho_powers.unwrap_or(vec![CF1::<C1>::zero(); 1]))
        })?;
        rho_powers.enforce_equal(&external_rho_powers)?;
...
pub mod tests {
    ...
    fn test_decider_circuit() {
        ...
        let hn_params = HN::preprocess(&mut rng, &(prep_param, 1, 1)).unwrap();

        // generate a Nova instance and do a step of it
        let mut hypernova = HN::init(&hn_params, F_circuit, z_0.clone()).unwrap();
        hypernova
            .prove_step(&mut rng, vec![], (vec![], vec![]))
            .unwrap();
