    ...
    <C as Group>::ScalarField: Absorb,
{
    ...
    pub fn compute_T(
        ...
        let t0 = thread::spawn(move || mat_vec_mul(&A_arc, &z1_arc));
        let z1_arc = Arc::from(z1);
        let t1 = thread::spawn(move || mat_vec_mul(&B_arc, &z1_arc));
        let z1_arc = Arc::from(z1);
        let t2 = thread::spawn(move || mat_vec_mul(&C_arc, &z1_arc));
        let A_arc = Arc::from(A.clone());
        let B_arc = Arc::from(B.clone());
        let C_arc = Arc::from(C.clone());
        let t3 = thread::spawn(move || mat_vec_mul(&A_arc, &z2_arc));
        let z2_arc = Arc::from(z2);
        let t4 = thread::spawn(move || mat_vec_mul(&B_arc, &z2_arc));
        let z2_arc = Arc::from(z2);
        let t5 = thread::spawn(move || mat_vec_mul(&C_arc, &z2_arc));
