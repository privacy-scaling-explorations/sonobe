...
pub trait Frontend<C: CurveGroup> {
...
impl<C: CurveGroup> Frontend<C> for ConstraintSystem<C::ScalarField> {
...
impl<E: Pairing> Frontend<E::G1> for CircomWrapper<E> {
...
mod frontend_tests {
    ...
    fn test_frontend_trait_logic<C: CurveGroup>(frontend: &dyn Frontend<C>, expect_success: bool, inputs: Vec<(String, Vec<BigInt>)>) {
        // R1CS
    ...
    fn load_circom_wrapper_as_frontend() -> Box<dyn Frontend<<Bn254 as Pairing>::G1>> {
    ...
    fn test_frontend_trait_for_circom() {
        let frontend = load_circom_wrapper_as_frontend();
    ...
    fn test_frontend_trait_for_arkworks() {
        ...
        let frontend: &dyn Frontend<_> = &cs; 
        
        let (r1cs, z_r1cs): (R1CS<Fr>, Vec<Fr>) = frontend.extract_r1cs_and_z(&[]).unwrap();

        r1cs.check_relation(&z_r1cs).unwrap();

        let (ccs, z_ccs): (CCS<_>, Vec<_>) = frontend.extract_ccs_and_z(&[]).unwrap();
